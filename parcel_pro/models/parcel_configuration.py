...
class ParcelConfiguration(models.Model):
    ...
    def post_contact(self,order,post_quotation):
        ...
        data_dict_partner = {
            ...
            "TelephoneNo": order.partner_id.phone or "",
        ...
        data_dict_shipping_partner = {
            ...
            "CompanyName": "",
            "ContactId": "",
            "Country": order.partner_id.Country or order.partner_shipping_id.country_id.code,
            "CustomerId": "",
            ...
            "State": order.partner_id.State or order.partner_shipping_id.state_id.code,
            "StreetAddress": str(order.partner_shipping_id.street) + str(order.partner_shipping_id.street2),
            "TelephoneNo": order.partner_shipping_id.phone,
            "Zip": order.partner_shipping_id.zip
        }
        print("data_dict_partner..",data_dict_partner)
        ...
        if not result.get("ContactId") or result.get("ContactId")=="NOID" or not result_shipping.get('ContactId'):
            raise ValidationError(_('ContactId not created on Parcel Pro for custom or for shipping address'))
        print("&&&&&&&&&&",result.get("ContactId"),result_shipping.get('ContactId'))
        ...
        order.partner_shipping_id.write({'CustomerId':result.get('CustomerId'),'ContactId':result.get('ContactId'),'ApartmentSuite':result.get('ApartmentSuite')})
        if post_quotation:
    ...
    def post_quotation(self,order,ContactId,ContactId_shipping):
        session = self.get_session()
        null = None
        true = True
        data_dict =  {
                       ...
                       "ServiceCode":"01-DOM",
                       "CarrierCode":2,
                       "ShipFrom": {"ContactId": "NOID"},
                       "ShipTo":{"ContactId":"NOID"},
